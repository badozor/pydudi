---
title: "**Working Notes:** Data Analysis with R, Python and Rcpp (version 0.999)"
author: "**P.BADY**"
...
library(utils)
library(tinytex)
options(encoding="UTF-8",prompt="  ", continue=" ",width = 85)
options(show.signif.stars=TRUE)
# non permanent command
load("/export/scratch2/data/TumorGrowth/.RData")
...
SourcePB('/export/scratch/GITprojectsOLD/methyltools/trunk/Rcode/preprocessFunnormv2.R')
SourcePB("/export/scratch/GITprojects/pbtools/trunk/Rcode/Rgraphics-0.1.R")
SourcePB("/export/scratch/GITprojectsOLD/statstools/trunk/Rcode/graphics-0.1.R")
gg_color_hue <- function(n) {
  hues = seq(15, 375, length = n + 1)
  hcl(h = hues, l = 65, c = 100)[1:n]
}
### additional personal functions
SourcePB("/export/scratch/Rplus/Rutility.R")
FigNB <- 0
FigCaption <- function(x){
  FigNB <<- FigNB +1
  paste("**Figure ",FigNB,":** ",x,sep="")
}
TabNB <- 0
TabCaption <- function(x){
  TabNB <<- TabNB +1
  paste("**Table ",TabNB,":** ",x,sep="")
}
RequirePB(circlize)
### color of the text
# color_block = function(color) {
#  function(x, options) sprintf('\\color{%s}\\begin{verbatim}%s\\end{verbatim}\\color{black}', color, x)
#  #function(x, options) sprintf('\\color{%s}\\baselinestretch{0.75}\\begin{footnotesize}\\begin{verbatim}%s\\end{verbatim}\\end{footnotesize}\\color{black}\\baselinestretch{1}', color, x)
#  #function(x, options) sprintf('\\color{%s}\\begin{footnotesize}\\begin{verbatim}%s\\end{verbatim}\\end{footnotesize}\\color{black}', color, x) 
# }
# knitr::knit_hooks$set(error = color_block('red'),
#                        output = color_block('Soutput'),
#                        source = color_block('Sinput'),
#                       warning = color_block('Soutput'),
#                       message = color_block('Soutput'))
```
...
plot(coordli[,1],coa1$li[,1],panel.first=c(grid()),xlab="Python (CS 1)",
     ylab="R (CS 1)",pch=19);abline(0,-1,col="red")
plot(coordli[,2],coa1$li[,2],panel.first=c(grid()),xlab="Python (CS 2)",
     ylab="R (CS 2)",pch=19);abline(0,-1,col="red")
```
...
plot(coordl1[,1],coa1$l1[,1],panel.first=c(grid()),xlab="Python (CS 1)",
     ylab="R (CS 1)",pch=19);abline(0,-1,col="red")
plot(coordl1[,2],coa1$l1[,2],panel.first=c(grid()),xlab="Python (CS 2)",
     ylab="R (CS 2)",pch=19);abline(0,-1,col="red")
```
...
plot(coordl1[,1],acm1$l1[,1],panel.first=c(grid()),xlab="Python (CS 1)",
     ylab="R (CS 1)",pch=19);abline(0,1,col="red")
plot(coordl1[,2],acm1$l1[,2],panel.first=c(grid()),xlab="Python (CS 2)",
     ylab="R (CS 2)",pch=19);abline(0,-1,col="red")
```
...
s.corcircle(pca2$co,sub="R version")
plot(test2$c1[,1],pca2$c1[,1],panel.first=c(grid()),xlab="RcppArmadillo (axis 1)",
     ylab="R (axis 1)",pch=19);abline(0,-1,col="red")
plot(test2$c1[,2],pca2$c1[,2],panel.first=c(grid()),xlab="RcppArmadillo (axis 2)",
     ylab="R (axis 2)",pch=19);abline(0,-1,col="red")
```
...
```{r fig.height=10,fig.width=10}
par(mfrow=c(2,2))
plot(test$X[,1],coa1$tab[,1],panel.first=c(grid()),xlab="Rcpp-Armadillo (col 1 of Z)",
     ylab="R (col 1 of tab)",pch=19);abline(0,1,col="red")
plot(test$c1[,1],coa1$c1[,1],panel.first=c(grid()),xlab="Rcpp-Armadillo (normed Axis 1)",
     ylab="R (normed Axis 1)",pch=19);abline(0,1,col="red")
plot(test$co[,1],coa1$co[,1],panel.first=c(grid()),xlab="Rcpp-Armadillo (Axis 1)",
     ylab="R (Axis 1)",pch=19);abline(0,1,col="red")
```
