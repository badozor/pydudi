...
def pydudi(X,cw,lw,nf):
  n = len(X)
  p = len(X.columns)
  D = np.diag(np.sqrt(lw[0]))
  Q = np.diag(np.sqrt(cw[0]))
  # XtDXQ
  XD = np.dot(X.T,D).T
  XD = np.dot(XD,Q)
  XtX = np.dot(XD.T,XD)
  ...
  # need to adjust the weighting
  L1 = np.dot(Li,np.diag(1/np.sqrt(eigenvalues)))
  Co = np.dot(C1,np.diag(np.sqrt(eigenvalues)))
...
# Normalize
X = X / X.std(ddof=0)
lw = pd.DataFrame(np.repeat(1/len(X),len(X)))
cw = pd.DataFrame(np.repeat(1,len(X.columns)))
